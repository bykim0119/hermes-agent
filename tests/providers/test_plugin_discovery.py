...
def test_all_33_profiles_register():
    """After discovery, the registry must contain exactly 33 bundled profiles.

    codex-exec is no longer a standalone bundled model-provider: it moved into
    the ``subagent_coder`` plugin and registers only when that plugin's
    ``register(ctx)`` runs (covered by
