...
from plugins.subagent_coder import _install_discord_coder_overlay
from plugins.subagent_coder import discord_overlay
...
def test_handle_message_routes_followup(monkeypatch):
    ...
    with patch(
        "plugins.subagent_coder.delegate_background.is_cancel_command",
        return_value=False,
    ):
...
def test_disconnect_unregisters_bus(monkeypatch):
    ...
    with patch(
        "plugins.subagent_coder.coder_event_bus.unregister_handler"
    ) as mock_unreg, patch(
        "plugins.subagent_coder.coder_sessions.get_global_sessions",
        return_value=None,
    ):
...
def test_post_connect_starts_flusher_and_registers(monkeypatch):
    ...
    with patch(
        "plugins.subagent_coder.coder_progress_formatter.DebouncedFlusher",
        return_value=fake_flusher,
    ), patch(
        "plugins.subagent_coder.coder_event_bus.register_handler"
    ) as mock_reg, patch(
        "plugins.subagent_coder.coder_sessions.set_global_sessions"
    ):
