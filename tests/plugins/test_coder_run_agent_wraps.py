...
def test_sequential_wrap_sets_parent_agent_contextvar(monkeypatch):
    from plugins.subagent_coder import _install_sequential_dispatch_wrap
...
def test_registry_handler_falls_back_to_contextvar(monkeypatch):
    ...
    import plugins.subagent_coder.delegate_background as _db
    ...
    with patch(
        "plugins.subagent_coder.delegate_background._spawn_detached_coder"
    ) as mock_spawn, patch(
        "plugins.subagent_coder.coder_config.check_codex_auth", return_value=None
    ):
...
def test_client_factory_wrap_returns_facade_for_codex_exec(monkeypatch):
    from plugins.subagent_coder import _install_codex_exec_client_factory_wrap
    ...
    with patch(
        "plugins.subagent_coder.codex_exec_client.CodexExecFacade",
...
def test_coder_spawn_callback_slot_installed():
    from plugins.subagent_coder import _install_coder_spawn_callback_slot
